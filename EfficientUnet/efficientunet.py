...
from PSP_layer.pyramid_pooling_module import PyramidPoolingModule
from SCSE_layer.scse import channel_spatial_squeeze_excite
from FeaturePyramidAttention.FPA_Keras import Feature_Pyramid_Attention
...
def _get_efficient_unet(encoder, met_shape, sat_shape, out_channels=24, 
                        concat_input=True, fpa=None, hypercolumn=True):
    met_input = Input(shape = met_shape)
    sat_input = Input(shape = sat_shape)
    MBConvBlocks = []
    ...
    o = UpBlock(128, initializer=conv_kernel_initializer, skip=blocks.pop(), met_input=met_input, sat_input=sat_input)(o)
    d3 = interpolation(o, 4)
    o = UpBlock(64, initializer=conv_kernel_initializer, skip=blocks.pop(), met_input=met_input, sat_input=sat_input)(o)
    d2 = interpolation(o, 2)
    if concat_input:
        o = UpBlock(32, initializer=conv_kernel_initializer, skip=blocks.pop(), met_input=met_input, sat_input=sat_input)(o)
    else:
    ...
    model = keras.models.Model(inputs=[encoder.input, met_input, sat_input], outputs=o)
    return model
